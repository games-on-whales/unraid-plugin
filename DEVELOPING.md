...
| `app-state.sh` | Sourced by `deploy.sh` and `update.sh` | Resolves the app run UID/GID and keeps Wolf's per-app state (and already-paired clients' saved UID/GID) matching it |
